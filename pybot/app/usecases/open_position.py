...
def open_position(dependencies: OpenPositionDependencies, input_data: OpenPositionInput) -> OpenPositionResult:
    ...
    try:
        ...
        fallback_base_qty = (
            submission.out_amount_atomic / SOL_ATOMIC_MULTIPLIER
            if entry_side == "BUY_SOL_WITH_USDC"
            else submission.in_amount_atomic / SOL_ATOMIC_MULTIPLIER
        )
        traded_base_sol = (
        ...
        actual_quote_usdc = (
            float(entry_result["spent_quote_usdc"])
            if "spent_quote_usdc" in entry_result and isinstance(entry_result["spent_quote_usdc"], (int, float))
        ...
        resolved_entry_price = (
            ...
            if "avg_fill_price" in entry_result and isinstance(entry_result["avg_fill_price"], (int, float))
        ...
        return OpenPositionResult(
            status="OPENED",
            trade_id=trade_id,
            summary=f"OPENED: tx={submission.tx_signature}, qty={qty_summary}, direction={direction}",
        )
    except Exception as error:
