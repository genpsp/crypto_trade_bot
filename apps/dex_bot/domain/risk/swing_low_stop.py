...
def calculate_swing_high(highs: list[float], lookback_bars: int) -> float:
    if lookback_bars <= 0:
        raise ValueError("lookback_bars must be greater than 0")
    if len(highs) < lookback_bars:
        raise ValueError(f"Not enough highs for swing high: required={lookback_bars}, actual={len(highs)}")

    recent_highs = highs[-lookback_bars:]
...
def calculate_max_loss_stop_price_for_short(entry_price: float, max_loss_pct: float) -> float:
    if entry_price <= 0:
        raise ValueError("entry_price must be greater than 0")
    if max_loss_pct <= 0:
        raise ValueError("max_loss_pct must be greater than 0")
...
def tighten_stop_for_short(entry_price: float, swing_high_stop: float, max_loss_pct: float) -> float:
    pct_stop = calculate_max_loss_stop_price_for_short(entry_price, max_loss_pct)
    return min(swing_high_stop, pct_stop)
...
def calculate_take_profit_price_for_short(entry_price: float, stop_price: float, r_multiple: float) -> float:
    if r_multiple <= 0:
        raise ValueError("r_multiple must be greater than 0")
    if entry_price >= stop_price:
        raise ValueError("entry_price must be less than stop_price for short")
