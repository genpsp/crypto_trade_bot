...
const SOL_ATOMIC_MULTIPLIER = 1_000_000_000n;
...
export class JupiterSwapAdapter implements ExecutionPort {
  ...
  async getMarkPrice(pair: string): Promise<number> {
    if (pair !== 'SOL/USDC') {
      throw new Error(`Unsupported pair for mark price: ${pair}`);
    }

    const quote = await this.quoteClient.fetchQuote({
      side: 'SELL_SOL_FOR_USDC',
      amountAtomic: SOL_ATOMIC_MULTIPLIER,
      slippageBps: 1,
      onlyDirectRoutes: false
    });
    const outUsdc = Number(quote.outAmountAtomic) / USDC_ATOMIC_MULTIPLIER;

    if (!Number.isFinite(outUsdc) || outUsdc <= 0) {
      throw new Error(`Invalid mark price quote: outAmountAtomic=${quote.outAmountAtomic.toString()}`);
    }
